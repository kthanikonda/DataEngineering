...
success_dir = os.path.join(parent_dir, "success")
failure_dir = os.path.join(parent_dir, "failure")

os.makedirs(success_dir, exist_ok=True)
os.makedirs(failure_dir, exist_ok=True)

error_log_filename = os.path.join(failure_dir, f"error_log_{today}.txt")


with open(error_log_filename, 'w') as error_file:
    error_file.write(f"Error Log for {today}\n\n")
...
for vid in vehicle_ids:
    ...
    try:
        ...
        filename = os.path.join(success_dir, f"vehicle_{vid}_{today}.json")
        ...
        error_msg = f"Vehicle ID {vid} failed with HTTPError: {e.code} - {e.reason}\n"
        ...
        error_msg = f"Vehicle ID {vid} failed with URLError: {e.reason}\n"
        with open(error_log_filename, 'a') as error_file:
            error_file.write(error_msg)
        print(error_msg.strip())
        failure_count += 1

with open(os.path.join(success_dir, "count.txt"), 'w') as f:
    f.write(f"Total Success Files: {success_count}\n")

with open(os.path.join(failure_dir, "count.txt"), 'w') as f:
    f.write(f"Total Failure Entries: {failure_count}\n")
