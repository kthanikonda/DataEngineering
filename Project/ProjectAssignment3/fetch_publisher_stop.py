...
parent_dir = f"stop_event_data_{today}"
...
def fetch_and_parse_html(vehicle_id):
    ...
    try:
        ...
        trip_id = trip_heading.text.split()[-1] if trip_heading else None
...
for vid in vehicle_ids:
    records = fetch_and_parse_html(vid)

    if records:
        filename = os.path.join(parent_dir, f"stop_{vid}_{today}.json")
        with open(filename, 'w') as f:
            json.dump(records, f, indent=2)

        print(f" Saved: {filename} - Publishing {len(records)} stop events...")
...
with open("count.txt", 'a') as f:
    ...
    f.write(f"Elapsed time: {end_time - start_time:.2f} seconds\n")

print("\nData Collection and Publishing Complete:")
print(f"   Success files: {success_count}")
print(f"   Stop events published: {msg_count}")
print(f"   Total time: {end_time - start_time:.2f} seconds")
print(f"   Files organized under: {parent_dir}")
print("   Summary written to: count.txt")
