...
for vid in vehicle_ids:
    ...
    try:
        ...
        logging.error(f"Error fetching/publishing data for vehicle_id {vid}: {e}")

# Wait for all Pub/Sub messages to be published
wait(futures)
end_time = time.time()

with open(os.path.join(parent_dir, "count.txt"), 'a') as f:
    f.write(f"Total Success Files: {success_count}\n")
    f.write(f"Total Breadcrumbs Published: {msg_count}\n")
